    ...
    def forward(self, X, w, y):
        ...
        Returns:
            loss - (float) The calculated loss normalized by the number of
                examples, N.
        """
        loss = np.mean(.5 * np.square(y - (np.dot(X, w))))
    ...
    def backward(self, X, w, y):
        ...
        Returns:
            gradient - (np.array) The (d+1)-dimensional gradient of the loss
                ...
                the bias term.
        """
        gradient = (y - (np.dot(X, w)))
        gradient = -np.mean([X[i] * gradient[i] for i in range(len(X))], axis=0)
        if self.regularization:
    ...
    def forward(self, X, w, y):
        ...
        Returns:
            loss - (float) The calculated loss normalized by the number of
                examples, N.
        """
        loss = np.mean(np.maximum(0, 1 - y * (np.dot(X, w))))
        if self.regularization:
            loss += self.regularization.forward(w)
        return loss
    ...
    def backward(self, X, w, y):
        ...
        Returns:
            gradient - (np.array) The (d+1)-dimensional gradient of the loss
                ...
                the bias term.
        """
        gradient = np.mean([-y[i] * X[i] if 1 - y[i]*np.dot(w.T, X[i]) > 0 else np.zeros(X.shape[1]) for i in range(X.shape[0])], axis=0)
        if self.regularization:
            gradient += self.regularization.backward(w)
        return gradient
