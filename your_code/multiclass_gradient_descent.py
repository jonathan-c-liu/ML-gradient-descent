...
class MultiClassGradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        self.model = [0 for x in range(len(self.classes))]

        for c in range(len(self.classes)):
            gd = GradientDescent(loss=self.loss, regularization=self.regularization, learning_rate=self.learning_rate,
                        reg_param=self.reg_param)
            OVAtargets = np.where(targets == self.classes[c], 1, -1)
            gd.fit(features, OVAtargets, batch_size=batch_size, max_iter=max_iter)
            self.model[c] = gd
    ...
    def predict(self, features):
        ...
        Returns:
            predictions - (np.array) A 1D array of predictions of length N,
                ...
                features.
        """
        conf = self.model[0].confidence(features)
        conf = np.reshape(conf, (conf.shape[0], 1))

        for i in range(np.size(self.classes)):
            if i == 0:
                continue
            col =  self.model[i].confidence(features)
            col = np.reshape(col, (col.shape[0], 1))
            conf = np.append(conf, col, axis=1)
        conf = np.argmax(conf, axis=1)

        predictions = np.zeros(np.size(conf))
        for i in range(np.size(conf)):
            predictions[i] = self.classes[int(conf[i])]
